...
画面は「パネル」の組み合わせで構成する。パネルは 3 種類。

| 種類 | 表示名 | 内容 |
|---|---|---|
| `thumbnails` | Thumbnails | 絞り込み + サムネイル一覧 |
| `preview` | Preview | プレビュー + 詳細 + 評価 |
| `import` | Import Settings | 取り込みの既定設定（§9.2） |
...
- 外部ウィンドウをメインに戻す方法:
  - 外部ウィンドウ自体（タイトルバー）をドラッグしてメインウィンドウの上（周囲 32px を含む）で離す。重なっている間はメインウィンドウに「離すと戻します」と表示する
  - 外部ウィンドウを閉じる（パネルは消えずにメインへ戻る）
  - 外部ウィンドウのアイコン、または Windows 一覧のアイコン
- ウィンドウのドラッグによる結合の判定（`ui/src/windowing/docking.ts`）:
  - 外部ウィンドウ側が自分の移動を監視する（Tauri: `onMoved`、ブラウザ: 位置のポーリング）。判定には外部ウィンドウのタイトルバー中央の点を使う（ドラッグ中のポインタ位置の近似）。メインウィンドウの隣に置いただけでは結合しない
  - OS のタイトルバーのドラッグには「離した」イベントがないため、重なった状態で 500ms 移動が止まったら離したとみなす
  - 開いた直後にメインウィンドウと重なっていても結合しないよう、一度メインウィンドウの外に出るまで判定しない。外部ウィンドウはメインウィンドウの右隣（モニター内に収まる位置）に開く
  - 座標は Tauri では物理ピクセル（両ウィンドウとも同じ単位）
- メインウィンドウを閉じると、外部ウィンドウもすべて閉じる（Rust の `on_window_event` とブラウザの `pagehide` の両方で実施）。
- ウィンドウのラベル: メインは `main`、外部は `pane-*`。`capabilities/default.json` はこの 2 つを対象にし、画面側からのウィンドウ作成（`core:webview:allow-create-webview-window`）、`close`、`set_focus` を許可する（位置・サイズ・モニターの取得は `core:default` に含まれる）。外部ウィンドウは `index.html?pane=<kind>` で開く。
...
| 取り込み設定 | バックエンド | 保存したバックエンドが `importSettingsUpdated` を送る |
| 取り込みの進行状況 | バックエンド | 実行中のバックエンドが `importProgress` を送る |
...
### 9.2 取り込み設定と取り込み（Task 3.1、モック段階）

Import Settings パネルで既定の設定を編集し、ステータスバーの「取り込み」ボタンから取り込む。設定は変更するたびに保存し、全ウィンドウで共有する（DTO: `ImportSettings`）。

- 保存する形式: 「全てに同じ設定を適用」または「星ごとに個別に設定」（未評価、★1〜★5 の 6 項目）。形式は「RAW のみ」「JPG + RAW」「JPG のみ」。HEIF は JPG と同じ扱い（現像済みの画像）。指定した形式のファイルがない撮影（例: JPG のみの設定で RAW しかない）は何もコピーしない。
- 保存先フォルダ: テキスト入力と「参照…」（Tauri のフォルダ選択ダイアログ。ブラウザのモックでは無効）。
- 保存先フォルダの構成:
  - 簡単設定: 日付（`{yyyy}-{MM}-{dd}`）、時間（`{HH}`）、星（`star{star}`）、RAW と JPG を別フォルダ（`{file}`）のチェックボックス。この順に入れ子にする
  - 高度な設定: テンプレートを直接入力。簡単設定から切り替えると、そのときの構成をテンプレートとして引き継ぐ
  - ファイル名はカードと同じ（テンプレートはフォルダだけ）
- テンプレート記法（`ui/src/importing/template.ts`）:
  - `/` でフォルダを区切る。変数は波かっこ: `{yyyy}` `{yy}` `{MM}` `{dd}` `{HH}` `{mm}` `{ss}` `{star}`（0〜5、未評価は 0）`{file}`（RAW / JPG / HEIF / VIDEO / META）
  - 波かっこにしたのは、`yymmdd` のような記法だとフォルダ名の文字（例: `mm`）と区別できないため
  - 検証: 不明な変数、閉じていない `{`、対応しない `}`、Windows で使えない文字（`<>:"\|?*`、制御文字）、`\`（区切りは `/`）、`.` / `..`、末尾の `.` や空白
  - 撮影日時が不明な場合、日時の変数は `unknown`
  - パスの区切りは保存先の書き方に合わせる（`D:\Photos` なら `\`）
- 取り込みダイアログ: 評価（全て / 未評価 / ★1〜★5）とメディア形式（全て / 画像 / 動画 / メタデータ）のチェックボックス。「全て」は個別の項目と連動し、一部だけ選ぶと中間状態になる。対象の件数・ファイル数・容量を表示し、開始できない理由（保存先なし、テンプレートの誤り、何も選ばれていない、実行中など）を示す。Esc で閉じ、Tab はダイアログ内を循環する。対象は選択中のデバイスの全アセット。
- メタデータ: `MediaKind::Sidecar`（XMP、将来は Sony の XML など）。画像の保存形式の設定とは関係なく、メタデータを選んだときにコピーする。
